...
export const IDBUserSchema = z.object({
  cep: z.string().regex(/^\d{8}$/, "CEP deve ter 8 dígitos sem formatação"),
  cpf: z
    .string()
    .length(11, "CPF deve ter exatamente 11 dígitos")
    .refine(validaDigitosCPF, { message: "CPF inválido" }),
  data_nascimento: z.string().refine((s) => !Number.isNaN(Date.parse(s)), {
  ...
  telefone: z
    ...
    .regex(
      /^\+?\d{10,15}$/,
      "Telefone deve ter entre 10 e 15 dígitos, podendo começar com +",
    ),
  tipo_usuario: z
    .number()
